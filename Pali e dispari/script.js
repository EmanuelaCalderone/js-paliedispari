...
console.log(pariDispari(numeroUtente, numeroComputer));

//creo la funzione con due argomenti
function pariDispari(numeroUtente, numeroComputer) {
    //creo la condizione per stabilire se la somma dei due numeri è pari o dispari
    if ((numeroUtente + numeroComputer) % 2 === 0) {
        console.log("la somma dei due numeri è un numero pari");
        if (sceltaUtente === "pari") {
            return ("Ha vinto l'utente")
        }
    }
    else {
        console.log("la somma dei due numeri è un numero dispari");
